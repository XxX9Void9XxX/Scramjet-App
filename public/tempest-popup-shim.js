(() => {
	...
	function fakePopup(baseUrl) {
		...
		const nav = (u) => {
			if (closed || !u) return;
			send({ type: "navigate", url: abs(u, baseUrl) });
		};
		...
		const doc = {
			open() { opened = true; buf = ""; },
			write(chunk) {
				if (!opened) { opened = true; buf = ""; }
				buf += String(chunk || "");
			},
			close() {
				...
				send({ type: "srcdoc", html: buf || "<!doctype html><title>Tempest</title>" });
			}
		};
	...
	if (nativeOpen) {
		window.open = function (url = "", target = "_blank", features = "") {
			...
			if (t === "_blank" || t === "_new" || t === "") {
				...
				return fakePopup(location.href);
			}
	...
	document.addEventListener("click", (event) => {
		const a = event.target instanceof Element ? event.target.closest("a[target='_blank']") : null;
		if (!a) return;
